...
# one set of artists per robot key
_brne_all = {}      # key -> LineCollection
_brne_opt = {}      # key -> Line2D
_dwa_all  = {}      # key -> LineCollection
_dwa_opt  = {}      # key -> Line2D

def visualize_brne(robot_state, goal, ped_list, ctl, ax=None, color='black', cfg=None, key=None):
    ...
    # choose a stable key per robot
    if key is None:
        key = color  # fallback, but better to pass robot['id']

    if cfg.get('all_samples', True):
        lines = [traj for traj in samples]
        lc = _brne_all.get(key)
        if lc is None:
            lc = LineCollection(lines, colors=color, linewidths=1, alpha=0.3)
            ax.add_collection(lc)
            _brne_all[key] = lc
        else:
            lc.set_segments(lines)

    # remove only THIS robot’s optimal line
    opt = _brne_opt.get(key)
    if opt is not None:
        opt.remove()
        _brne_opt[key] = None
    ...
    if cfg.get('optima', True):
        ...
        if w0.sum() > 0:
            ...
            opt_line, = ax.plot(traj_star[:, 0], traj_star[:, 1],
                                color=color, linewidth=2.5, zorder=10)
            _brne_opt[key] = opt_line

    ax.plot(goal[0], goal[1], 'gx', markersize=6, zorder=10)


def visualize_dwa(robot_state, goal, obs, ctl, ax=None, color='black', cfg=None, key=None):
    ...
    if cfg.get('optima', True):
        ...
        xs, ys = zip(*best_traj)
        opt_line, = ax.plot(xs, ys, color=color, linewidth=2.5, zorder=10)
        _dwa_opt[key] = opt_line
